...
class RecognizationController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($q)
                ...
                ->addColumn('action', fn($r)=>
                    '<a href="'.route('recognizations.edit',$r->id).'" class="btn btn-sm btn-info">Edit</a>
    ...
    /* ================= EDIT ================= */
    public function edit(Recognization $recognization)
    {
        return view('admin.recognization.edit', compact('recognization'));
    }

    public function update(Request $request, Recognization $recognization)
    {
        $request->validate([
            'title'   => 'required',
            'image'   => 'nullable|image|max:2048',
    ...
    /* ================= DELETE ================= */
    public function destroy(Recognization $recognization)
    {
        if ($recognization->image) {
            Storage::disk('public')->delete([
                $recognization->image,
